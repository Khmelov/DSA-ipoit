...
public class A_BinaryFind {
    ...
    int[] findIndex(InputStream stream) throws FileNotFoundException {
        ...
        int[] a = new int[n];
        for (int i = 0; i < n; i++) { // Цикл от 0 до n-1 для Java-массива
            a[i] = scanner.nextInt();
        }

        //размер массива для поиска
        ...
        for (int i = 0; i < k; i++) {
            ...
            while (left <= right) {
                ...
                if (a[mid] == valueToFind) {
                    // Нашли! Индекс по условию задачи - это mid + 1
                    result[i] = mid + 1;
                    found = true;
                    break; // Выходим из цикла, т.к. элемент найден
                } else if (a[mid] < valueToFind) {
                    // Искомый элемент в правой половине
                    left = mid + 1;
                } else { // a[mid] > valueToFind
                    // Искомый элемент в левой половине
                    right = mid - 1;
                }
            }

            // Если после цикла элемент не был найден
            if (!found) {
                result[i] = -1;
            }
        }
