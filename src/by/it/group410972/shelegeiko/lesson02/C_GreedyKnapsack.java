...
public class C_GreedyKnapsack {
    ...
    double calc(InputStream inputStream) throws FileNotFoundException {
        ...
        // Сортируем предметы по удельной стоимости (стоимость за единицу веса).
        Arrays.sort(items);
        ...
        for (Item item : items) {
            if (currentWeight + item.weight <= W) {
                ...
                // Иначе, берем только часть предмета, чтобы заполнить рюкзак.
                int remainingWeight = W - currentWeight;
                result += ((double) remainingWeight / item.weight) * item.cost;
                currentWeight = W; // Рюкзак заполнен.
                break; // Больше ничего не поместится.
            }
        }
